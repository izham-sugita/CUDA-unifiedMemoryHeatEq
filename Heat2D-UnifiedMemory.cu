...
/*Not sure where the memory allocater resides*/
#include<cuda.h>
...
int main()
{
  ...
  cout<<"Step dum: "<<iter+steps<<endl;
  operation += 7.0*(double)ny *(double)nx*(double)steps;
  iter +=steps;
    
  }while(iter<itermax-steps+1);
  ...
  ofstream fileOut;
  fileOut.open("/mnt/RAM_disk/cudaUnifiedMemHeatEq.csv");
  fileOut<<"x,y,z,T\n";
  for(int i=0; i<nx; ++i){
    for(int j=0; j<ny; ++j){
      int id = i*ny + j;
